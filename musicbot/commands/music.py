...
class Music(commands.Cog):
    ...
    async def _play_song(self, ctx, *, track: str):
        ...
        if audiocontroller.playlist.loop == True:
            await ctx.send("Повторение включено! Используй {}loop Чтобы выключить".format(config.BOT_PREFIX))
            return
    ...
    async def _loop(self, ctx):
        ...
        if audiocontroller.playlist.loop == False:
            ...
            await ctx.send("Повторение включено! :arrows_counterclockwise:")
        else:
            audiocontroller.playlist.loop = False
            await ctx.send("Повторение выключено :x:")
    ...
    async def _move(self, ctx, *args):
        if len(args) != 2:
            ctx.send("Неверное количество аргументов")
            return
        ...
        try:
            ...
            await ctx.send("Неправильное положение")
            return
    ...
    async def _skip(self, ctx):
        ...
        await ctx.send("Пропущенна текущая песня :fast_forward:")
    ...
    async def _volume(self, ctx, *args):
        ...
        try:
            ...
            await ctx.send("Ошибка: Объем должен быть числом 1-100")
